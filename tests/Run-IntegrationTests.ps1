...
$testEmail = "test_runner_$timestamp@gmail.com"
$testPassword = "TestPass$timestamp"

Write-Host ""
Write-Host "  [SETUP] Dang ky tai khoan test: $testEmail" -ForegroundColor DarkGray

$regResult = Invoke-Api -Method POST -Endpoint "/api/auth/register" -Body @{ email = $testEmail; password = $testPassword; role = "Student"; otp = "123456" }
...
$loginResult = Invoke-Api -Method POST -Endpoint "/api/auth/login" -Body @{ email = $testEmail; password = $testPassword } -Session $session
$loginOk = $loginResult.Success -and $loginResult.Data.success -eq $true -and ($null -ne ($session.Cookies.GetCookies($BaseUrl) | Where-Object { $_.Name -eq "unimap360.accessToken" }))
...
# Test 1.4: GET /api/auth/me với token/session hợp lệ
$meResult = Invoke-Api -Method GET -Endpoint "/api/auth/me" -Session $session
...
# Test 1.5: GET /api/auth/me không có token/session
$meNoToken = Invoke-Api -Method GET -Endpoint "/api/auth/me"
Assert-Test "1.5 GET /me khong co token/session tra ve 401" ($meNoToken.StatusCode -eq 401)

# Test 1.6: POST /api/auth/refresh với session hợp lệ
$refreshResult = Invoke-Api -Method POST -Endpoint "/api/auth/refresh" -Session $session
$refreshOk = $refreshResult.Success -and $refreshResult.Data.success -eq $true
...
if ($firstJobId) {
    ...
    Assert-Test "3.2 GET /jobs/{id} (SKIP - khong co job trong DB)" $true "Khong tim thay Job ID trong listings"
}
...
# Test 4.1: GET /api/notifications (có session)
$notiList = Invoke-Api -Method GET -Endpoint "/api/notifications" -Session $session
...
$notiSummary = Invoke-Api -Method GET -Endpoint "/api/notifications/summary" -Session $session
